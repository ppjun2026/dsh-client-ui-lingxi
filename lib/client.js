...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    const TASK_PRIORITY_META = {
      high: { label: '高', color: 'var(--dsw-alias-state-error-primary)', hex: '#ff6b6b' },
      medium: { label: '中', color: 'var(--dsw-alias-state-warn-primary)', hex: '#ffb454' },
      low: { label: '低', color: 'var(--dsw-alias-label-tertiary)', hex: '#8b8b9e' },
    }
    ...
    const ui = {
      ...
      selectedId: null,
      editingTask: null, // { projectId, taskId }
      dragTask: null,    // { projectId, taskId }
      search: '',
      statusFilter: 'all',
      tagFilter: 'all',
      domainFilter: 'all',
    ...
    const CSS = `
      ...
      .lx-task {
        ...
        background: var(--dsw-alias-bg-layer-2); border: 1px solid var(--dsw-alias-border-l1);
        border-radius: 8px; margin-bottom: 6px; font-size: 12.5px; cursor: grab;
        overflow: hidden; transition: border-color 120ms, opacity 120ms;
      }
      .lx-task:hover { border-color: var(--dsw-alias-border-l2); }
      .lx-task-pri { width: 3px; flex: none; }
      .lx-task-content { flex: 1; min-width: 0; padding: 8px 6px 8px 10px; }
      .lx-task-title { font-weight: 500; line-height: 1.4; overflow-wrap: anywhere; }
      .lx-task-meta { display: flex; align-items: center; gap: 8px; margin-top: 4px; }
      .lx-task-pri-label { font-size: 10.5px; font-weight: 600; }
      .lx-task-due { font-size: 10.5px; color: var(--dsw-alias-label-tertiary); }
      .lx-task-due.overdue { color: var(--dsw-alias-state-error-primary); font-weight: 600; }
      .lx-task-ops { display: flex; align-items: center; gap: 2px; padding: 4px 6px; flex: none; }
      .lx-task select { background: transparent; border: none; color: inherit; font-size: 11px; cursor: pointer; outline: none; }
      .lx-task.lx-dragging { opacity: 0.5; }
      .lx-col.drag-over { outline: 2px dashed var(--dsw-alias-state-business-primary); outline-offset: -2px; border-radius: 10px; }
      .lx-seg { display: flex; gap: 6px; }
      .lx-seg-btn {
        flex: 1; padding: 7px 0; border: 1px solid var(--dsw-alias-border-l2);
        border-radius: 8px; background: transparent; color: var(--dsw-alias-label-secondary);
        cursor: pointer; font-size: 12.5px; font-weight: 600;
        transition: background 120ms, color 120ms, border-color 120ms;
      }
      .lx-seg-btn:hover { background: var(--dsw-alias-interactive-bg-hover); }
      .lx-seg-btn.selected {
    ...
    function render(container) {
      ...
      if (overlay) {
        overlay.innerHTML = ui.editingTask ? renderTaskEditor(ui.editingTask) : (ui.selectedId ? renderDetail(ui.selectedId) : '')
      }
    }
    ...
    function renderTaskEditor(ref) {
      const project = ui.data.projects.find(p => p.id === ref.projectId)
      if (!project) return ''
      const task = project.tasks.find(t => t.id === ref.taskId)
      if (!task) return ''
      const prioritySeg = Object.entries(TASK_PRIORITY_META).map(([k, v]) =>
        `<button type="button" class="lx-seg-btn ${task.priority === k ? 'selected' : ''}" data-action="pick-task-priority" data-p="${k}" style="--pc:${v.color}">${v.label}</button>`
      ).join('')
      return `
        <div class="lx-overlay" data-action="close-editor">
          <div class="lx-modal" data-stop="true" style="width:min(440px,92%)">
            <div class="lx-modal-head">
              <div class="lx-title">编辑任务</div>
              <button class="lx-btn" data-action="close-editor">取消</button>
            </div>
            <div class="lx-modal-body" data-role="task-editor">
              <div class="lx-section">
                <div class="lx-section-h">任务标题</div>
                <input class="lx-input" style="width:100%" placeholder="任务标题…" value="${escapeHtml(task.title)}" data-role="task-title">
              </div>
              <div class="lx-section">
                <div class="lx-section-h">优先级</div>
                <div class="lx-seg">${prioritySeg}</div>
              </div>
              <div class="lx-section">
                <div class="lx-section-h">截止日期</div>
                <input class="lx-input" type="date" style="width:100%" value="${escapeHtml(task.dueDate || '')}" data-role="task-due">
              </div>
              <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:4px">
                <button class="lx-btn primary" data-action="save-task">保存</button>
              </div>
            </div>
          </div>
        </div>`
    ...
    function renderProjects() {
      ...
      return projects.map(p => {
        ...
        const cols = ['todo', 'doing', 'done'].map(status => {
          ...
          return `<div class="lx-col" data-dropzone="${status}" data-project="${escapeHtml(p.id)}">
            <div class="lx-col-h" style="color:${meta.color}">${meta.label}（${list.length}）</div>
            ${list.map(t => {
              const pri = TASK_PRIORITY_META[t.priority] || TASK_PRIORITY_META.medium
              const overdue = t.dueDate && t.status !== 'done' && t.dueDate < todayStr()
              return `<div class="lx-task" draggable="true" data-action="edit-task" data-project="${escapeHtml(p.id)}" data-task="${escapeHtml(t.id)}" title="点击编辑 · 拖拽到其他列">
                <span class="lx-task-pri" style="background:${pri.hex}"></span>
                <div class="lx-task-content">
                  <div class="lx-task-title">${escapeHtml(t.title)}</div>
                  <div class="lx-task-meta">
                    <span class="lx-task-pri-label" style="color:${pri.color}">${pri.label}</span>
                    ${t.dueDate ? `<span class="lx-task-due ${overdue ? 'overdue' : ''}">${overdue ? '⚠ 逾期 ' : ''}${escapeHtml(t.dueDate)}</span>` : ''}
                  </div>
                </div>
                <div class="lx-task-ops">
    ...
    function createGraph(canvas, ideas, onSelect) {
      ...
      // 标签 halo：主题背景色半透明块，保证文字在边/节点之上清晰可读
      const haloColor = colorToRgba(readCssVar('--dsw-alias-bg-base', '#16161c'), 0.85)
      ...
      function draw() {
        ...
        for (const n of nodes) {
          ...
          if (n.overall > 0) {
            const scoreStr = String(n.overall)
            ctx.font = `600 ${Math.round(10 / zoom)}px sans-serif`
            const sw = ctx.measureText(scoreStr).width
            // 评分小圆底 halo
            ctx.beginPath()
            ctx.arc(n.x, n.y, Math.max(r * 0.6, sw / 2 + 4 / zoom), 0, Math.PI * 2)
            ctx.fillStyle = haloColor
            ctx.fill()
            ctx.fillStyle = dimmed ? dimColor : labelColor
            ctx.textAlign = 'center'
            ctx.textBaseline = 'middle'
            ctx.fillText(scoreStr, n.x, n.y)
          }
          // 标签：缩放过小时隐藏文字避免拥挤；正常时绘制主题色 halo 保证可读性
          if (zoom >= 0.5) {
            const label = n.title.length > 14 ? n.title.slice(0, 14) + '…' : n.title
            const fontSize = Math.round(11 / zoom)
            ctx.font = `${isHover ? 700 : 400} ${fontSize}px sans-serif`
            ctx.textAlign = 'center'
            ctx.textBaseline = 'top'
            const labelY = n.y + r + 4 / zoom
            const tw = ctx.measureText(label).width
            const th = fontSize
            ctx.beginPath()
            ctx.roundRect(n.x - tw / 2 - 6 / zoom, labelY - 3 / zoom, tw + 12 / zoom, th + 6 / zoom, 5 / zoom)
            ctx.fillStyle = haloColor
            ctx.fill()
            ctx.fillStyle = dimmed ? dimColor : labelColor
    ...
    function bindPanelEvents(container) {
      container.addEventListener('click', (event) => {
        ...
        switch (action) {
          ...
          case 'edit-task': setUI({ editingTask: { projectId: target.dataset.project, taskId: target.dataset.task } }); break
          case 'close-editor': setUI({ editingTask: null }); break
          case 'save-task': handleSaveTask(container); break
          case 'pick-task-priority': {
            const seg = target.closest('.lx-seg')
            if (seg) {
              seg.querySelectorAll('.lx-seg-btn').forEach(b => b.classList.remove('selected'))
      ...
      container.addEventListener('click', (event) => {
        ...
        if (overlay && event.target === overlay) {
          setUI({ selectedId: null, editingTask: null })
        }
      })
      ...
      container.addEventListener('dragstart', (event) => {
        if (event.target.closest('select, button, .lx-task-ops')) return
        const task = event.target.closest('.lx-task')
        if (!task || !task.dataset.project || !task.dataset.task) return
        ui.dragTask = { projectId: task.dataset.project, taskId: task.dataset.task }
        event.dataTransfer.effectAllowed = 'move'
        event.dataTransfer.setData('text/plain', task.dataset.task)
        requestAnimationFrame(() => task.classList.add('lx-dragging'))
      })
      container.addEventListener('dragend', () => {
        ui.dragTask = null
        container.querySelectorAll('.lx-col.drag-over').forEach(z => z.classList.remove('drag-over'))
      })
      container.addEventListener('dragover', (event) => {
        const zone = event.target.closest('[data-dropzone]')
        if (!zone || !ui.dragTask) return
        event.preventDefault()
      ...
      container.addEventListener('drop', (event) => {
        ...
        const { projectId, taskId } = ui.dragTask
    ...
    // ── 具体操作 ────────────────────────────────────────────────────────────
    function handleSaveTask(container) {
      const editor = container.querySelector('[data-role="task-editor"]')
      if (!editor || !ui.editingTask) return
      const title = editor.querySelector('[data-role="task-title"]').value.trim()
      if (title === '') { window.alert('任务标题不能为空'); return }
      const priority = editor.querySelector('.lx-seg-btn.selected')?.dataset?.p || 'medium'
      const dueDate = editor.querySelector('[data-role="task-due"]').value || ''
      const { projectId, taskId } = ui.editingTask
      setUI({ editingTask: null })
      doAction({ kind: 'update-task', projectId, taskId, patch: { title, priority, dueDate } })
    }
