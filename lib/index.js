...
function normalizeProject(value) {
  ...
  return {
    ...
    tasks: Array.isArray(value.tasks) ? value.tasks.filter(isRecord).map(t => ({
      ...
      dueDate: isString(t.dueDate) ? t.dueDate : '',
    })).filter(t => t.id !== '') : [],
    createdAt: Number.isFinite(value.createdAt) ? value.createdAt : now(),
    updatedAt: Number.isFinite(value.updatedAt) ? value.updatedAt : now(),
  }
  ...
  switch (action.kind) {
    ...
    case 'add-task': {
      ...
      project.tasks.push({
        ...
        priority: TASK_PRIORITIES.includes(action.input.priority) ? action.input.priority : 'medium',
        dueDate: isString(action.input.dueDate) ? action.input.dueDate : '',
      })
      project.updatedAt = ts
      break
    }
    case 'update-task': {
      ...
      if (TASK_PRIORITIES.includes(action.patch.priority)) task.priority = action.patch.priority
      if (isString(action.patch.dueDate)) task.dueDate = action.patch.dueDate
      project.updatedAt = ts
      break
    }
    case 'delete-task': {
